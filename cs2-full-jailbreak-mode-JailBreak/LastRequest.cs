...
internal class LastRequest
{
    ...
    {
        if (!Player.IsValidPlayer(player)) return false;
    ...
    public static void StartLRS4S(CCSPlayerController? t, CCSPlayerController? ct)
    {
        ...
        if (!Player.IsValidPlayer(t, requireAlive: true) || !Player.IsValidPlayer(ct, requireAlive: true)) return;
        if (t!.PlayerPawn?.Value?.WeaponServices == null || ct!.PlayerPawn?.Value?.WeaponServices == null) return;
    ...
    public static void BulletCycleLRS4S(CCSPlayerController? player)
    {
        ...
        if (JailBreak.LRPlayer == null || JailBreak.LRPlayerCT == null) return;
        if (player!= JailBreak.LRPlayer && player != JailBreak.LRPlayerCT) return;

        var lrPawn = JailBreak.LRPlayer.PlayerPawn?.Value;
        var lrCtPawn = JailBreak.LRPlayerCT.PlayerPawn?.Value;
        if (lrPawn?.WeaponServices?.ActiveWeapon?.Value == null || lrCtPawn?.WeaponServices?.ActiveWeapon?.Value == null) return;

        if (player == JailBreak.LRPlayerCT)
        { 
            lrPawn.WeaponServices.ActiveWeapon.Value.ReserveAmmo[0] = 1;
            Utilities.SetStateChanged(lrPawn.WeaponServices.ActiveWeapon.Value, "CBasePlayerWeapon", "m_pReserveAmmo");
        }
        else
        {
            lrCtPawn.WeaponServices.ActiveWeapon.Value.ReserveAmmo[0] = 1;
            Utilities.SetStateChanged(lrCtPawn.WeaponServices.ActiveWeapon.Value, "CBasePlayerWeapon", "m_pReserveAmmo");
        }
    ...
    public static void LRNoScopeAtScope(CCSPlayerController player)
    {
        ...
        pawn.ItemServices?.As<CCSPlayer_ItemServices>().RemoveWeapons();
        player.GiveNamedItem("weapon_awp");
        if (pawn.WeaponServices.ActiveWeapon?.Value == null) return;
        pawn.WeaponServices.ActiveWeapon.Value.Clip1 = 0;
        Utilities.SetStateChanged(pawn.WeaponServices.ActiveWeapon.Value, "CBasePlayerWeapon", "m_iClip1");
